...
export function Header() {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    // Optionally clear the query or keep it:
    // setSearchQuery("")
  }
  // ----------------------------------
  ...
  {navigation.map((item) => {
    ...
    return (
      <Link
        key={item.name}
        href={item.href}
        className={`px-5 py-3 text-sm font-medium rounded-full transition-all duration-200 border ${
          isActive
            ? "bg-amber-100/90 text-amber-800 border border-amber-200 shadow-sm"
      ...
      >
        ...
        <div className="hidden lg:flex lg:items-center lg:space-x-1">
          {/* --- Desktop search form (WRAPPED in form to submit on Enter) --- */}
          <SearchBar />
